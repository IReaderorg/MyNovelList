...
const GET = async ({ request, params }) => {
  ...
  const { data, error } = await supabase.from("novels").select("*").eq("id", params.id).eq("user_id", auth.userId).single();
  if (error) {
...
const DELETE = async ({ request, params }) => {
  ...
  const { error } = await supabase.from("novels").delete().eq("id", params.id).eq("user_id", auth.userId);
  if (error) {
