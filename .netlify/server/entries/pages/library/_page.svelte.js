import { c as create_ssr_component, d as add_attribute, e as escape, b as each, f as createEventDispatcher, v as validate_component, a as subscribe } from "../../../chunks/ssr.js";
...
const NovelCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="card hover:border-gray-600 transition-colors group"><div class="flex gap-4"> <div class="flex-shrink-0 w-20 h-28 bg-gray-700 rounded-lg overflow-hidden">${novel.cover_url && true ? `<img${add_attribute("src", novel.cover_url, 0)}${add_attribute("alt", novel.title, 0)} class="w-full h-full object-cover" loading="lazy">` : `<div class="w-full h-full flex items-center justify-center text-gray-500" data-svelte-h="svelte-mbp1ee"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg></div>`}</div>  <div class="flex-1 min-w-0"><div class="flex items-start justify-between gap-2"><div class="min-w-0"><a href="${"/novel/" + escape(novel.id, true)}" class="block"><h3 class="font-semibold text-gray-100 truncate hover:text-primary-400 transition-colors">${escape(novel.title)}</h3></a> ${novel.author ? `<p class="text-sm text-gray-400 truncate">${escape(novel.author)}</p>` : ``}</div> <span class="${"flex-shrink-0 px-2 py-0.5 text-xs rounded-full text-white " + escape(statusColors[novel.status], true)}">${escape(statusLabels[novel.status])}</span></div>  ${novel.tags && novel.tags.length > 0 ? `<div class="mt-1 flex flex-wrap gap-1">${each(novel.tags.slice(0, 3), (tag) => {
    return `<span class="px-1.5 py-0.5 text-xs bg-gray-700 text-gray-400 rounded">${escape(tag)}</span>`;
  })} ${novel.tags.length > 3 ? `<span class="text-xs text-gray-500">+${escape(novel.tags.length - 3)}</span>` : ``}</div>` : ``}  <div class="mt-2 flex items-center gap-2"><label class="text-sm text-gray-400" data-svelte-h="svelte-1kq7ao">Ch.</label> <input type="number" min="0" class="w-16 px-2 py-1 text-sm bg-gray-700 border border-gray-600 rounded text-gray-100 focus:outline-none focus:ring-1 focus:ring-primary-500"${add_attribute("value", chapterInput, 0)}> ${novel.total_chapters ? `<span class="text-sm text-gray-400">/ ${escape(novel.total_chapters)}</span>` : ``}</div>  ${novel.total_chapters ? `<div class="mt-2 h-1.5 bg-gray-700 rounded-full overflow-hidden"><div class="h-full bg-primary-500 transition-all duration-300" style="${"width: " + escape(getProgressPercent(), true) + "%"}"></div></div>` : ``}  ${novel.score !== void 0 && novel.score !== null ? `<div class="mt-2 flex items-center gap-1"><svg class="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg> <span class="text-sm text-gray-300">${escape(novel.score)}</span></div>` : ``}</div></div></div>`;
});
...
const NovelForm = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<form class="space-y-4">${validate_component(Input, "Input").$$render(
      $$result,
      {
        id: "title",
        label: "Title *",
      ...
      {}
    )} <div class="grid grid-cols-3 gap-4">${validate_component(Input, "Input").$$render(
      $$result,
      {
        id: "current_chapter",
      ...
      {}
    )}</div> <div class="grid grid-cols-2 gap-4"><div><label for="started_at" class="block text-sm font-medium text-gray-300 mb-1" data-svelte-h="svelte-1dketpd">Started Date</label> <input id="started_at" type="date" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"${add_attribute("value", started_at, 0)}></div> <div><label for="completed_at" class="block text-sm font-medium text-gray-300 mb-1" data-svelte-h="svelte-f1qq55">Completed Date</label> <input id="completed_at" type="date" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"${add_attribute("value", completed_at, 0)}></div></div> ${validate_component(Input, "Input").$$render(
      $$result,
      {
        id: "tags",
      ...
      {}
    )} <div><label for="notes" class="block text-sm font-medium text-gray-300 mb-1" data-svelte-h="svelte-1k1yhi1">Notes</label> <textarea id="notes" rows="3" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none" placeholder="Personal notes...">${escape(notes || "")}</textarea></div> <div class="flex justify-end gap-3 pt-2">${validate_component(Button, "Button").$$render($$result, { variant: "secondary" }, {}, {
      default: () => {
        return `Cancel`;
      }
      ...
      {
        default: () => {
          return `${escape(loading ? "Saving..." : novel ? "Update" : "Add Novel")}`;
        }
      }
    )}</div></form>`;
  } while (!$$settled);
