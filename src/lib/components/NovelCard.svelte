<script lang="ts">
	...
	// Update local state when novel changes
...
<div class="card hover:border-gray-600 transition-colors group">
	<div class="flex gap-4">
		<!-- Cover -->
		<div class="flex-shrink-0 w-20 h-28 bg-gray-700 rounded-lg overflow-hidden relative">
		...
		<div class="flex-1 min-w-0">
			<div class="flex items-start justify-between gap-2">
				...
				{#if novel.progress}
					<span class="flex-shrink-0 px-2 py-0.5 text-xs rounded-full text-white {statusColors[novel.progress.status]}">
						{statusLabels[novel.progress.status]}
					</span>
				{/if}
			</div>
			...
			{#if novel.progress}
				...
				{#if novel.progress.score !== undefined && novel.progress.score !== null}
					<div class="mt-2 flex items-center gap-1">
						...
						<span class="text-sm text-gray-300">{novel.progress.score}</span>
					</div>
				{/if}
			{/if}
		</div>
